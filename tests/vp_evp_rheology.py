...
def vp_evp_test1(timescale=10, timestep=10 ** (-1), number_of_triangles=30, rheology="VP", advection=False,
    ...
    lm = inner(beta * rho * h * (u - u_) / timestep + rho_w * C_w * sqrt(dot(u - ocean_curr, u - ocean_curr)) * (
                u - ocean_curr), v) * dx
    ...
    if not advection:
        if rheology == "VP" and solver == "FE":
            ...
            all_u, all_h, all_a = mevp_solver(u, u_, lm, t, timestep, subcycle, bcs, sigma, ep_dot, P, zeta, T,
                                              timescale,
                                              pathname='./output/vp_evp_test/vp_test1mevp.pvd', output=output)
        elif rheology == "EVP" and solver == "EVP":
            all_u, all_h, all_a = evp_solver(u, u_, lm, t, timestep, subcycle, bcs, sigma, ep_dot, P, zeta, T,
                                             timescale,
    ...
    if advection:
        if rheology == "VP" and solver == "FE":
            all_u, all_h, all_a = forward_euler_solver(u, u_, lm, bcs, t, timestep, timescale,
                                                       ...
                                                       output=output, advection=advection, lh=lh, la=la, h=h, h_=h_,
                                                       a=a, a_=a_)
        elif rheology == "VP" and solver == "mEVP":
            all_u, all_h, all_a = mevp_solver(u, u_, lm, t, timestep, subcycle, bcs, sigma, ep_dot, P, zeta, T,
                                              timescale,
