...
class SeaIceModel(object):
    def __init__(self, mesh, conditions, timestepping, params, output, solver_params):
        ...
        self.U1 = FunctionSpace(mesh, 'DG' ,0)
...
class ElasticViscousPlastic(SeaIceModel):
    def __init__(self, mesh, conditions, timestepping, params, output, solver_params):
        ...
        rheology = params.e ** 2 * sh + Identity(2) * 0.5 * (1 - params.e ** 2) * tr(sh) + 0.5 * self.Ice_Strength(h, a)
...
class ElasticViscousPlasticTransport(SeaIceModel):
    def __init__(self, mesh, conditions, timestepping, params, output, solver_params):
        ...
        rheology = params.e ** 2 * sh + Identity(2) * 0.5 * (1 - params.e ** 2) * tr(sh) + 0.5 * self.Ice_Strength(hh, ah)
