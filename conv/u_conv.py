...
def plot_u_conv_vs_stab(timescale,timestep):
    ...
    t = np.arange(0, timescale, timestep)
    plt.semilogy(t, strain_rate_tensor_error(timescale,timestep,number_of_triangles=100,stabilised=0),'r--',label = r'$n = 100, \sigma = \frac{\zeta}{2}(\nabla v + \nabla v^T)$')
    plt.semilogy(t,strain_rate_tensor_error(timescale,timestep,number_of_triangles=10,stabilised=0),'b.',label = r'$n = 10, \sigma = \frac{\zeta}{2}(\nabla v + \nabla v^T)$')
    plt.semilogy(t,strain_rate_tensor_error(timescale,timestep,number_of_triangles=100,stabilised=2),'g--',label = r'$n = 100, \sigma = \frac{\zeta}{2}(\nabla v)$')
    plt.semilogy(t,strain_rate_tensor_error(timescale,timestep,number_of_triangles=10,stabilised=2),'k.',label = r'$n = 10, \sigma = \frac{\zeta}{2}(\nabla v)$')
    plt.legend(loc='best')
    plt.show()
